...
// 代码来源，chainlink官方文档：https://docs.chain.link/ccip/tutorials/send-arbitrary-data
...
contract NFTPoolLockAndRelease is CCIPReceiver, OwnerIsCreator {
    ...
    /**
     * 当前NFT已经被锁在这个合约池子里面了
     * @param tokenId 被锁的NFT tokenId
     * @param newOwner 在目标池子里面 mint 一个新的NFT，owner是 newOwner
     * @param chainSelector 发送的信件
     * @param receiver 收货地址
     * 
     * contract a: function1 -> msg.sender
     * contract b: function2 -> call func1
     * call func1 msg.sender = users'address
    ...
    function lockAndSendNFT(
        ...
        address receiver) public returns(bytes32) {
            //transfer NFT to this address to lock the NFT Pool
            //发送人，接受者
            //这里是合约B调用这个方法，所以，需要合约A持有人给B这个权限
